...
public class RegexContext {
	...
	public enum ContextKey {
		...
		private List<Object> contextRange;
		
		private ContextKey(int first, int last, int labelIndex) {
			this.contextRange = new ArrayList<Object>();
			if (first != -1) {
				if (last == -1) {
					...
					for (int i=first; i<=last; i++) {
						this.contextRange.add(WordKey.values()[i]);
					}
				}
			}
			else if (labelIndex != -1) {
				this.contextRange.add(LabelKey.values()[labelIndex]);
			}
		}
	...
	public void addNotRegex(ContextKey context, Pattern regex) {
		List<Pattern> list = new ArrayList<Pattern>();
		if (contextNotMap.containsKey(context)) {
			list = contextNotMap.get(context);
		}
		if (list != null) {
			list.add(regex);
		}
		
		contextNotMap.put(context, list);
	}
	
	
	public void addNotRegex(ContextKey context, List<Pattern> regexList) {
		List<Pattern> list = new ArrayList<Pattern>();
		if (contextNotMap.containsKey(context)) {
			list = contextNotMap.get(context);
		}
	...
	public void addNotRegexMap(Map<ContextKey, List<Pattern>> newMap) {
		contextNotMap.putAll(newMap);
	}
	...
	//TODO: add function to create multi-word string to compare against regex
	private String getCompareString(List<CoreLabel> tokensSpan, ) {
